...
### #63 outcome (2026-08-22)

Audit item 10 ("admin surface routes have no auth/role checks — any visitor with the URL can drive scrape jobs, approve/reject pending businesses, read the dashboard and analytics") closed with two sequential, gated commits: **bd288fb** (server) — every admin-surface handler now starts from `createAuthMiddleware(["admin"])` + `createAuthErrorResponse` (the #55 auth foundation): `scrape-jobs` GET+POST, `pending-businesses` GET, `pending-businesses/import` POST, `import/job/[jobId]` POST, `businesses/[id]/approve` + `reject` + `bulk-approve` POST, `admin/dashboard` GET, `analytics/scrape-jobs` + `recent` GET, and `users` GET; `users` PATCH keeps its existing manual `verifyTokenSafe` + `payload.role !== "admin"` check (envelope unification is #65). Eight route specs mock `@/lib/auth/jwt-middleware` and each gained a 401 test. **5f97b78** (client) — role now flows login/register resolvers → `ClientUser.role` → localStorage session; the admin console, reviews, and scrape pages each run a guard effect (no session → `/login`, non-admin → `/owner`), every admin API fetch carries `authHeaders()`, and load-type fetches clear the session + redirect on 401; `UserManagement.tsx` got `authHeaders()` on its three fetches; e2e utils capture `role` from the login response and seed it into `black-owned.session`; the two admin page specs mock `next/navigation` (the scrape spec's GET assertion now expects the headers arg). Gates: tsc clean; full jest 86 suites / 1289 tests (1281 + 8 new 401 tests, no collateral loss); live probes on `/api/admin/dashboard` — 401 no header, 403 `Insufficient permissions` for a non-admin JWT, 200 with the full counts payload for an admin JWT (test user promoted via psql, then reverted); Playwright on chromium — admin-console 3/3, approval-workflow 4/4, claim-wizard 2/2, chat 3/3 (the admin-console suite now exercises the real role guard end-to-end: psql-promoted admin, fresh login, seeded session with `role: "admin"`). Two documented deferrals: the `UserManagement` PATCH targets `/api/users/role` and `/api/users/status`, which do not exist as routes (only `PATCH /api/users` does) — pre-existing UI/API drift, out of scope here; and the inline per-route guard style was chosen deliberately over a shared helper to match the existing owner-route pattern.

### Owner-reported gap: social profile discovery (2026-08-22)

Outside the ticket scope (the LOC tickets never specced it, so the code-vs-ticket audit could not flag it): `black_wall_street` (the prior app) shipped a **social profile discovery** feature that was never ported. Components there: `scripts/social-discovery/` Python pipeline (Phase 1 — extract social links from the business website; Phase 2 — Google search `"{name} {platform} {city} {state}"` with name-match verification + confidence scoring; Phase 3 — direct URL probing via handle variations with not-found detection) across six platforms (**Facebook, Instagram, TikTok, Twitter/X, LinkedIn, YouTube**); a per-business `social_urls` field (URL + confidence per platform); and a "Social Media Section" on the business detail page with platform embeds. The new app has none of it: no `social_urls` storage, no discovery pipeline, no social section on the detail page. Owner decision (2026-08-22): port it in full — all 6 platforms, pipeline + storage + detail-page section — tracked as task #70, scheduled after the audit sweep.
